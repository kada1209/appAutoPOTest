...
import static base.InitAppium.appPackage;
...
/**
 * @author wangxinqi
 * @date 2019-04-08 09:52
 *
 * 逻辑操作类，逻辑功能类继承这个类，在这里获取页面控件使用page层的对象
 */
public class OperateAppium {
    ...
    public void swipeToUp(int during, int percent) {
        ...
        tAction.press(PointOption.point(width/2,height*(percent-1)/percent))
                .waitAction(WaitOptions.waitOptions(Duration.ofSeconds(during)))
                .moveTo(PointOption.point(width/2,height/percent))
                .release().perform();
    }
    ...
    public void swipeToDown(int during, int percent) {
        ...
        TouchAction tAction = new TouchAction(driver);
        tAction.press(PointOption.point(width/2,height/percent)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(during)))
                .moveTo(PointOption.point(width/2,height*(percent-1)/percent))
                .release().perform();
    }
    ...
    public void swipeToLeft(int during, int percent) {
        ...
        TouchAction tAction = new TouchAction(driver);
        tAction.press(PointOption.point(width*(percent-1)/percent,height/2)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(during)))
                .moveTo(PointOption.point(width/percent,height/2))
                .release().perform();
    ...
    public void swipeToRight(int during, int percent) {
        ...
        TouchAction tAction = new TouchAction(driver);
        tAction.press(PointOption.point(width/percent,height/2)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(during)))
                .moveTo(PointOption.point(width*(percent-1)/percent,height/2))
                .release().perform();
    }
    ...
    public void press(AndroidElement ae) {
        try {
            ...
            Dimension dimension = ae.getSize();
            int x = point.getX();
           int y  = point.getY();
            tAction.tap(PointOption.point(x,y));
        } catch (Exception e) {
    ...
    public void press(int x, int y) {
        try {
            TouchAction tAction = new TouchAction(driver);
            tAction.tap(PointOption.point(x,y)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)));
    ...
    public void longPress(AndroidElement ae) {
        try {
            ...
            int x = point.getX();
            int y  = point.getY();
            tAction.longPress(PointOption.point(x,y)).release().perform();
        } catch (Exception e) {
    ...
    public void swipOnElement(AndroidElement element, String direction, int duration) {
        ...
        switch (direction) {
            case SWIP_UP:
                ...
                TouchAction tAction = new TouchAction(driver);
                tAction.press(PointOption.point(startX,y+height*4/5)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(duration)))
                        .moveTo(PointOption.point(startX,y+height/5))
                        .release().perform();
                break;
            case SWIP_DOWN:
                startX = x + width / 2;
                ...
                tAction1.press(PointOption.point(startX,y+height/5)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(duration)))
                        .moveTo(PointOption.point(startX,y+height*4/5))
                        .release().perform();
            ...
            case SWIP_LEFT:
                ...
                tAction2.press(PointOption.point(x+width*4/5,startY)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(duration)))
                        .moveTo(PointOption.point(x+width/5,startY))
                        .release().perform();
                break;
            case SWIP_RIGHT:
                startY = y + width / 2;
                //driver.swipe(x + width / 5, startY, x + width * 4 / 5, startY, duration);
                TouchAction tAction3 = new TouchAction(driver);
                tAction3.press(PointOption.point(x+width/5,startY)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(duration)))
                        .moveTo(PointOption.point(x+width*4/5,startY))
                        .release().perform();
                break;
        }
    }
